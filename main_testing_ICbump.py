    ...
    for i in range(I):
        for j in range(J):
            if -np.pi/2<lambdas[i]<np.pi/2:
                Utest0[j,i]=500*np.sin(lambdas[i])*(1-mus[j]**2)
                Vtest0[j,i]=500*np.cos(lambdas[i])*mus[j]*np.sqrt((1-mus[j]**2))
    
    Umic0=rfl.fwd_fft_trunc(Utest0, I, M)  
    Vmic0=rfl.fwd_fft_trunc(Vtest0, I, M)            
    newetaic0,newdeltaic0,etamnic0,deltamnic0=rfl.diagnostic_eta_delta(Umic0,Vmic0, fmn,I,J,M,N,Pmn,Hmn,w,tstepcoeff,mJarray,dt)
    Uic=Utest0
    Vic=Vtest0
    etaic0=np.real(newetaic0)
    deltaic0=np.real(newdeltaic0)
