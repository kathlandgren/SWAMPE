...
spinupdata[1,0] = spinupdata[0,0]
spinupdata[1,1] = spinupdata[0,1]
...
for t in range(2,tmax):
    ...
    if test==1:
        # For Test 1 in Williamson et al (1992), the winds are reset at every time step
        newU=Uic
        newV=Vic
    ...
    spinupdata[t,0] = np.min(np.sqrt(Udata[t,:,:]**2 + Vdata[t,:,:]**2 ))
    ...
    if spinupdata[t,1]>8000:
        print('Time stepping stopped due to wind blow up. Max RMS winds = '+str(spinupdata[t,1]))
        t=tmax
